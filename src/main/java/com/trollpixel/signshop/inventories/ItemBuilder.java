...
public class ItemBuilder implements Cloneable {
    ...
    public ItemBuilder lore(boolean override, String... lore) {
        ...
        for (String s : lore) {
            lines.add(MessageUtils.translateColorCodes(s));
        }
        ...
        for (int i = 0; i < lines.size() - 1; i++) {
            ...
            Matcher nextMatcher = APIConstants.Patterns.START_COLOR_PATTERN.matcher(lines.get(i + 1));

            if (nextMatcher.find()) {
                continue;
            }

            Matcher currentMatcher = APIConstants.Patterns.COLOR_PATTERN.matcher(line);
        ...
        for (int i = 0; i < lines.size(); i++) {
            ...
            if (line != null && !line.isEmpty() && !APIConstants.Patterns.START_COLOR_PATTERN.matcher(line).find()) {
                lines.set(i, ChatColor.GRAY + line);
            }

            if (line != null && ChatColor.stripColor(line).isEmpty()) {
                lines.set(i, "");
            }
        }
    ...
    public Integer nbtInt(String tag) {
        return nbt(compound -> compound.hasKey(tag) ? compound.getInt(tag) : null);
    }

    public Long nbtLong(String tag) {
        return nbt(compound -> compound.hasKey(tag) ? compound.getLong(tag) : null);
    }

    public Double nbtDouble(String tag) {
        return nbt(compound -> compound.hasKey(tag) ? compound.getDouble(tag) : null);
    }

    public Boolean nbtBoolean(String tag) {
        return nbt(compound -> compound.hasKey(tag) ? compound.getBoolean(tag) : null);
    }
